...
import { Plus, Trash2, Newspaper, X, Upload, Loader2 } from "lucide-react";
...
export default function AdminArticles() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!imageFile) return alert("Silakan pilih gambar (thumbnail) terlebih dahulu");
            ...
            <div key={item.id} className="bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm flex flex-col group relative">
              <div className="aspect-video bg-gray-100 relative">
                ...
                <button 
                  ...
                  className="absolute top-4 right-4 p-2 bg-white/90 text-red-500 hover:bg-red-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                >
                  <Trash2 size={16} />
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-lg font-black text-gray-900">Tulis Artikel Baru</h2>
